...
import { 
  ...
  Easing,
  Keyboard,
  FlatList // Changed for Web
} from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { GestureHandlerRootView } from 'react-native-gesture-handler';
// DraggableFlatList removed for web stability
...
// Placeholder för bild - På webben fungerar require annorlunda, vi använder en dummy om filen inte finns
// I webbläsaren kan vi inte enkelt require:a lokala assets utan webpack config, men detta är en placeholder.
const BACKGROUND_IMAGE = { uri: 'https://via.placeholder.com/1080x1920.png?text=Abstract+Background' };
...
export default function App() {
  ...
  // const isDraggingRef = useRef(false); // Inte relevant för webb utan dnd
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(floatAnim, {
          ...
          useNativeDriver: true, // På webben stödjer RNW detta
        }),
        Animated.timing(floatAnim, {
          toValue: 0,
          duration: 2000,
  ...
  const loadUserAndLogin = (username: string) => {
    ...
    if (Platform.OS !== 'web') Keyboard.dismiss(); // Web doesn't always need dismiss
  };
  ...
  useEffect(() => {
    timerIntervalRef.current = setInterval(() => {
      // removed isDragging check for web
  ...
  // --- RENDER TRACKER FOR WEB (NO DRAG) ---
  ...
  const renderSignUpView = () => {
    ...
    return (
      <View style={{
        ...
        backgroundColor: theme.background, // Fallback if image fails
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1, backgroundColor: theme.background, height: '100%' }}>
...
const styles = StyleSheet.create({
  ...
  modeTab: { flex:1, alignItems:'center', padding:8, borderRadius:6, cursor: 'pointer' },
  ...
  arrowButton: { width:40, height:40, borderRadius:20, borderWidth:2, alignItems:'center', justifyContent:'center', cursor: 'pointer' },
  timerValueText: { fontSize:30, fontWeight:'bold' },
  actionButton: { padding:15, borderRadius:12, alignItems:'center', cursor: 'pointer' },
  actionButtonText: { color:'#fff', fontWeight:'bold', fontSize:16 },
  progressBarBg: { height:4, borderRadius:2, marginBottom:10, overflow:'hidden' },
  progressBarFill: { height:'100%' },
  sectionTitle: { fontSize:18, fontWeight:'bold', marginBottom:15 },
  themeCard: { flexDirection:'row', padding:15, borderRadius:12, borderWidth:1, marginBottom:10, alignItems:'center', cursor: 'pointer' },
  ...
  colorCircle: { width:36, height:36, borderRadius:18, cursor: 'pointer' },
  ...
  filterBtn: { flex:1, padding:8, alignItems:'center', borderRadius:6, cursor: 'pointer' },
